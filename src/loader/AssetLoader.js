...
core.inherits(AssetLoader, Object, {
     ...
     * @param [format=core.FORMAT.JSON] {Number} The format of the map data.
     * @param [overwrite=false] {Boolean} If an entry with a matching key already exists this will over-write it.
     */
     ...
     * @param [format=core.FORMAT.XML] {core.FORMAT} The format of the bitmap font data.
     * @param [overwrite=false] {Boolean} If an entry with a matching key already exists this will over-write it.
     */
    ...
    fileComplete: function(key) {
        if(!this.assets[key])
            return core.utils.warn('fileComplete key is invalid!', key);
